...
Having established the systems advantage, we verify it does not come at the cost of inference quality: across calibration and matched-budget posterior recovery, the streaming estimator's inference quality is comparable to that of the matched synchronous baseline in low dimensions and under multimodality, degrading only in four dimensions, where both under-cover and the asynchronous method somewhat more.

\paragraph{Calibration.} Table~\ref{tab:sbc} reports SBC empirical coverage on the Gaussian-mean model (1000 trials). The asynchronous method tracks the nominal level closely ($0.50/0.78/0.87/0.92$ at the $0.50/0.80/0.90/0.95$ levels); the synchronous baseline under-covers at every level ($0.38/0.64/0.76/0.83$), i.e.\ it is over-confident. At 1000 trials a coverage estimate carries a Monte Carlo standard error of only ${\approx}\,\pm0.007$--$0.016$, well below the ${\approx}0.09$--$0.14$ gap between the methods, so the ordering is resolvable rather than noise: on this benchmark the streaming estimator is \emph{better} calibrated than the matched synchronous baseline. It still under-covers slightly at the two highest levels ($0.87$ vs.\ $0.90$ and $0.92$ vs.\ $0.95$, several binomial standard errors), so we report it as substantially better calibrated than the baseline with residual upper-level under-coverage, not as exactly nominal. This test is on a one-dimensional model with an analytic posterior; the higher-dimensional and multimodal settings where the archive/proposal approximations are most likely to bite are probed separately below. Figure~\ref{fig:sbc} shows the corresponding coverage curve and rank histogram.

\paragraph{Calibration under multimodality and in higher dimensions.} To probe exactly those settings, we run SBC (1000 trials, matched simulation budget) on two harder targets. The first is a \emph{multimodal} model $y_i\sim\mathcal N(\theta^2,\sigma^2)$: the sign ambiguity gives a symmetric bimodal posterior at $\pm\sqrt{\bar y}$, the case a top-$k$ archive is most at risk of collapsing onto a single mode. It does not collapse --- the archive retains \emph{both} modes in $85.7\%$ of trials and the mode containing the true value in $86.7\%$ (the remainder are largely near-$\theta{=}0$ trials where the two modes merge), the importance weights stay near-uniform (median effective-sample-size fraction $0.99$, median maximum normalised weight $0.011$), and coverage tracks the nominal level ($0.48/0.79/0.88/0.93$), calibrated within binomial error at the two lower levels with the same mild upper-level under-coverage seen in one dimension. The second is the \emph{four-dimensional} g-and-k model. Here both methods under-cover at the upper levels under the matched budget, and the asynchronous method more so (e.g.\ empirical coverage $0.85/0.89$ at the $0.90/0.95$ levels for the location parameter $A$, and lower for the shape parameters $B,k$); its rank distributions depart from uniform (chi-square $p<10^{-3}$) while the weights remain healthy (median effective-sample-size fraction $0.81$), so the miscalibration reflects an over-concentrated top-$k$ posterior in higher dimensions rather than weight degeneracy. Doubling the simulation budget (to $80{,}000$ per trial) leaves the coverage essentially unchanged --- $0.85/0.89$ at the $0.90/0.95$ levels for $A$, and within Monte Carlo error for $B,g,k$ --- so the under-coverage is \emph{structural}, not budget-limited: additional simulations refine the same top-$k$ core rather than populating the posterior tails. We therefore report the streaming estimator as well calibrated in one dimension and robust to multimodality, but \emph{over-confident in four dimensions}, a concrete limitation whose lever is a larger archive or the defensive-mixture floor (\S\ref{sec:limitations}) rather than more simulation.

\begin{table}[t]
\caption{SBC empirical coverage on Gaussian-mean inference (parameter $\mu$, 1000 trials; Monte Carlo standard error ${\approx}\,\pm0.007$--$0.016$). Closer to the nominal level is better.}\label{tab:sbc}
\centering
\begin{tabular}{@{}lcccc@{}}
\toprule
