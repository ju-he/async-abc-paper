...
\begin{assumption}[denominator class]\label{ass:class} $\Theta\subset\mathbb{R}^d$ is a non-degenerate axis-aligned box and the prior density satisfies $0<\pi_-\le\pi\le\pi_+<\infty$ on it. Each reconstructed proposal $q_{\tau_s}$ in \eqref{eq:snapshot-denominator} is a $\Theta$-truncated Gaussian mixture with at most $k$ components, means in $\Theta$, weights in the simplex, and covariance $\Sigma$ obeying $\sigma_-^2I\preceq\Sigma\preceq\sigma_+^2I$ for fixed $0<\sigma_-\le\sigma_+<\infty$.\end{assumption}
...
Assumption~\ref{ass:kernel} holds for the Gaussian kernel with $L_K=2/(e\epsilon_\infty)$ and Epanechnikov with $2/\epsilon_\infty$; it excludes the hard indicator, which is not Lipschitz in $\epsilon$ --- one concrete reason the method uses smooth kernels. Assumptions~\ref{ass:class} and \ref{ass:defensive} constrain only the \emph{reported denominator}, which the implementation builds to that exact recipe. Assumption~\ref{ass:stab} carries everything else, and \eqref{eq:fidelity} is the whole of the sampler's contribution: not that the sampler take any particular form, but that whatever it does be asymptotically reflected in the denominator, up to a ratio $r$ that the results below carry rather than assume away. Note the two clauses differ in strength: consistency permits $r$ and $\epsilon_\infty$ to be random, while the central limit theorem needs them deterministic, because the centred integrand must be a fixed function for the martingale arrays to be adapted. That restriction costs nothing in the case of most interest, $r\equiv1$, which is deterministic no matter how run-dependent $\tilde q_\infty$ is.

\begin{theorem}[consistency]\label{thm:consistency}
Let Assumptions~\ref{ass:kernel}--\ref{ass:defensive} and \ref{ass:stab}(i) hold. Then for every bounded measurable $f:\Theta\to\mathbb{R}$,
\begin{equation}
...
Let Assumptions~\ref{ass:kernel}--\ref{ass:defensive} and \ref{ass:stab}(i)--(ii) hold. Then for every bounded measurable $f$,
\begin{equation}
...
\label{eq:tilt-bound}
\end{equation}
More sharply, $\zeta$ may be replaced by $\zeta_0=\pi_{\epsilon_\infty}(|r-1|)$, which the reported weights estimate directly as $\widehat\zeta=\sum_i\widehat w_i\,|r(\theta_i)-1|$.
\end{proposition}

\subsection{What $r$ is for this implementation}\label{sec:theory-r}
Everything specific to the code enters through $r$, and it factorizes into four contributions, all of them ratios of densities and hence multiplicative.
...
\emph{(b) The snapshot quadrature.} The $m$-point mixture stands in for the $n$-point one, with error at most $V_n/m$ where $V_n=\sum_{i<n}\|q_{i+1}-q_i\|_\infty$ is the total variation of the proposal path. Fixing $m$ therefore trades cost against fidelity; see the remark below.
...
\ref{ass:class} denominator class & by construction, \emph{except} the normalizer & The prior support is a box and the archive lies in it, so component means do; the covariance carries an additive jitter $10^{-9}\mathrm{tr}(\widehat\Sigma)/d+10^{-12}\bar b^2$ ($\bar b^2$ the mean squared box width), whose second term is a run-independent floor, giving $\sigma_-^2>0$, and $\sigma_+^2$ follows from the archive lying in a bounded box. The truncation \emph{normalizer}, however, is the product of per-axis in-box masses --- exact for a diagonal covariance, increasingly accurate as a component's mass moves away from the faces relative to its own scale, an approximation in general --- so the evaluated component density is not exactly the one sampled from. \\
