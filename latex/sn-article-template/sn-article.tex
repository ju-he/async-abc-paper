...
\paragraph{Posterior recovery.} On the g-and-k and Lotka--Volterra benchmarks the asynchronous method reaches a \emph{lower} Wasserstein distance to the truth than the synchronous baseline at matched wall-clock budget, and on the expensive Cellular Potts simulator the two are comparable (Fig.~\ref{fig:posterior-recovery}); per-parameter posteriors and joint marginals are consistent with the known/reference values. In the straggler runs (which compute posterior quality) the final Wasserstein distances are comparable between methods (asynchronous $\approx0.07$--$0.09$ vs.\ synchronous $\approx0.07$). On the Gaussian-mean benchmark, which has a closed-form posterior, the asynchronous method recovers the posterior mean to within $\approx0.002$--$0.013$ absolute error---comparable to the synchronous baseline ($\approx0.005$--$0.022$) and far tighter than a rejection-ABC reference---and tracks the baseline's Wasserstein-vs-time trajectory throughout the budget (Fig.~\ref{fig:gaussian-recovery}); the SBC calibration above is unaffected.
