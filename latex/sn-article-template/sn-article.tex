...
Reverting the smooth kernel to a hard indicator degrades the estimator relative to the full method, while removing the AMIS cumulative-mixture reweighting is approximately neutral on this uniform-runtime analytic target (Fig.~\ref{fig:ablation}). On the Gaussian-mean benchmark the full method attains a final Wasserstein-to-truth of $0.073$ (CI $[0.067,0.079]$); the hard-indicator kernel raises it to $0.076$, whereas dropping AMIS leaves it essentially unchanged ($0.072$; Fig.~\ref{fig:ablation}a), with the remaining hyperparameter variants scattered around the full-method reference and degrading only toward the largest perturbation scales ($0.084$--$0.087$). That AMIS is neutral here is consistent with its role: its reweighting corrects the over-representation induced by \emph{runtime--parameter coupling} (\S\ref{sec:results-hpc}), which this uniform-runtime target does not exercise. The AMIS-isolation trajectory (Fig.~\ref{fig:ablation}b) accordingly shows the full method and the no-AMIS variant tracking each other throughout the converged regime (both $\approx 0.072$--$0.073$ after $40$\,s).
