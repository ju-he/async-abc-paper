...
\begin{assumption}[stabilization]\label{ass:stab} \emph{(i, consistency)} $\epsilon_n\downarrow\epsilon_\infty>0$ and
\begin{equation}
...
\end{equation}
$\bar q^\star_n$ being the mixture the pooled sample was actually drawn from. \emph{(ii, central limit theorem)} In addition, $\epsilon_\infty$ is \emph{deterministic}, a (possibly random) limiting proposal $q_\infty$ exists, with $\bar q_\infty=\delta\pi+(1-\delta)q_\infty$, and, almost surely,
\begin{equation}
\begin{aligned}
...
Let Assumptions~\ref{ass:kernel}--\ref{ass:defensive} and \ref{ass:stab}(i) hold for $\mathcal{A}_\delta$. Then for every bounded measurable $f:\Theta\to\mathbb{R}$,
$\widehat\pi_n(f)\to\pi_{\epsilon_\infty}(f)$ almost surely.
\end{theorem}

\begin{theorem}[central limit theorem]\label{thm:clt}
Let Assumptions~\ref{ass:kernel}--\ref{ass:defensive} and \ref{ass:stab}(i)--(ii) hold for $\mathcal{A}_\delta$. Then for every bounded measurable $f$,
\begin{equation}
