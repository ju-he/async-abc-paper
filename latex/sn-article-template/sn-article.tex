...
\begin{assumption}[kernel]\label{ass:kernel} $K_\epsilon:[0,\infty)\to[0,1]$ is measurable with $K_\epsilon(0)=1$ (unit \emph{peak}, not unit integral, as is standard for a probabilistic-acceptance ABC kernel), and there is $L_K<\infty$ with $|K_\epsilon(\rho)-K_{\epsilon'}(\rho)|\le L_K|\epsilon-\epsilon'|$ for all $\rho\ge0$ and $\epsilon,\epsilon'\in[\epsilon_{\mathrm{lo}},\epsilon_0]$, the deterministic bandwidth range of Assumption~\ref{ass:stab}(i). The limiting target is non-degenerate: $Z_{\epsilon_\infty}=\int_\Theta\pi L_{\epsilon_\infty}>0$.\end{assumption}
...
\begin{assumption}[fidelity and stabilization]\label{ass:stab} \emph{(i, consistency)} $\epsilon_n\downarrow\epsilon_\infty$ with $\epsilon_n\ge\epsilon_{\mathrm{lo}}$ throughout for a \emph{deterministic} $\epsilon_{\mathrm{lo}}>0$ (so $\epsilon_\infty\ge\epsilon_{\mathrm{lo}}>0$, though $\epsilon_\infty$ itself may be random), and the denominator tracks the draw mixture up to a limiting ratio: for some $r$ with $0<r_-\le r\le r_+<\infty$,
\begin{equation}
...
\]
The product telescopes for any positive intermediates, so the content is in the choice of intermediates, not the algebra: a different insertion order attributes the total differently, and the factors are a decomposition of one quantity rather than four independent errors.
...
$r_{\mathrm{floor}}=\bigl(\nu_\infty+(1-\nu_\infty)u\bigr)/\bigl(\delta+(1-\delta)u\bigr)$, so $r_{\mathrm{floor}}-1=(\delta-\nu_\infty)(u-1)/(\delta+(1-\delta)u)$: it is exactly $1$ when $\nu_\infty\ge\delta$, below $1$ wherever the proposal is thinner than the prior, and it vanishes as $\delta\to0$, i.e.\ as $m$ grows. This is the one contribution available in closed form.
...
Let $\phi:\Theta\to\mathbb{R}$ be bounded measurable, let $\ell\in\{1,2\}$ (an exponent, not the fidelity ratio $r$), and let $\epsilon\in[\epsilon_{\mathrm{lo}},\epsilon_0]$ and the positive function $q\ge\delta\pi$ be either deterministic or $\mathcal{F}_{i-1}$-measurable. Then under \eqref{eq:app-conditional-law}, writing $L^{(1)}_\epsilon:=L_\epsilon$,
\[
...
\midrule
\ref{ass:kernel} kernel, Lipschitz part & by construction & Gaussian ($L_K=2/e\epsilon_\infty$) and Epanechnikov ($2/\epsilon_\infty$) are peak-normalized and Lipschitz in $\epsilon$ on $[\epsilon_\infty,\epsilon_0]$. The hard indicator is not, and is excluded from the theory. \\
