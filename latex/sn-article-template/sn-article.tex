...
\label{eq:rate}
\end{equation}
where $\epsilon_{i-1}$ is the bandwidth in force at the $i$-th proposal and $\bar q_{i-1}$ is \eqref{eq:snapshot-denominator} formed from the first $i-1$ draws --- both $\mathcal{F}_{i-1}$-measurable. The second condition is an \emph{aggregate} rate on these predictable proxies; it is what lets the central limit theorem accommodate a random $q_\infty$.
\end{assumption}
...
\paragraph{Assumed versus true by construction.} Table~\ref{tab:assumption-status} in Appendix~\ref{app:theory} maps every assumption onto its status in the implementation. Assumptions~\ref{ass:kernel} and \ref{ass:defensive} hold by construction, the latter for the denominator but not (in the reported runs) for the sampler; Assumption~\ref{ass:class} holds up to the component normalizer. Assumption~\ref{ass:stab} is a genuine assumption: the top-$k$ archive rule does not obviously stabilize, since the archive keeps improving as better particles arrive, and we do not prove that it does. Its consequences are what \S\ref{sec:results-validity}'s simulation-based calibration probes empirically. Two further approximations are absorbed into it rather than left dangling: under asynchronous execution the reporting rank's arrival-ordered log need not reproduce the proposal each candidate was actually drawn from, and the $m$-snapshot mixture is a coarse quadrature of the proposal path --- both enter only through $\|\bar q_n-\bar q_\infty\|_\infty$.
...
Let $h:\Theta\to\mathbb{R}$ be bounded measurable and let $\epsilon\in[\epsilon_\infty,\epsilon_0]$ and the density $q\ge\delta\pi$ be either deterministic or $\mathcal{F}_{i-1}$-measurable. Then under \eqref{eq:app-conditional-law},
\[
...
\end{lemma}
\begin{proof}
Condition on $\mathcal{F}_{i-1}$, given which $\epsilon$ and $q$ are constants, and integrate first over $\rho_i\sim p(\cdot\mid\theta_i)$, which gives the factor $L_\epsilon(\theta_i)=\mathbb{E}[K_\epsilon(\rho)\mid\theta_i]$, then over $\theta_i\sim q^\delta_i$. The $\mathcal{F}_{i-1}$-measurable case is what Steps~2 and 3 of the proof of Theorem~\ref{thm:clt} need; an $\mathcal{F}_\infty$-measurable $q$ --- such as the limit $\bar q_\infty$ --- would \emph{not} be admissible here, which is why that proof centres on predictable proxies rather than on the limit. Fubini applies because the integrand is bounded by $\|h\|_\infty/\delta$ (Lemma~\ref{lem:bounded}).
\end{proof}
...
\emph{Step 2 (drift).} Lemma~\ref{lem:condmean} applies to $V_i$ because $(\epsilon_{i-1},\bar q_{i-1})$ is $\mathcal{F}_{i-1}$-measurable, and $\int\pi gL_{\epsilon_\infty}=0$, so
\[
...
\emph{(b) Conditional variance.} Lemma~\ref{lem:condmean} applied to $\pi^2g^2K^2_{\epsilon_{i-1}}/\bar q^2_{i-1}$ (the same conditioning, with $K^2_\epsilon$ integrating against $p(\cdot\mid\theta)$ to give $L^{(2)}_\epsilon$) gives
\[
