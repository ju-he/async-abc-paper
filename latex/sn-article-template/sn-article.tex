...
The analysis is organized around a single question, because that is all the proofs turn out to need: \emph{is the denominator the estimator divides by the density its sample was actually drawn from?} Write $\tilde q_i$ for the conditional density of the $i$-th candidate given everything realized before it was proposed. This object exists for the implementation whatever its internals do --- the top-$k$ archive rule, the capped in-box rejection, the underflow redraws, the bootstrap phase --- and it is the \emph{only} way the sampler enters what follows. Write
\begin{equation}
...
\begin{assumption}[fidelity and stabilization]\label{ass:stab} \emph{(i, consistency)} $\epsilon_n\downarrow\epsilon_\infty>0$, and the denominator tracks the draw mixture up to a limiting ratio: for some $r$ with $0<r_-\le r\le r_+<\infty$,
\begin{equation}
...
Everything specific to the code enters through $r$. To attribute it, insert three intermediates between the density actually sampled from and the denominator actually divided by: $\bar q^{(1)}$, the same mixture with exact rather than product-of-marginal truncation constants; $\bar q^{(2)}$, that mixture with the capped rejection and underflow redraws removed; and $\bar q^{(3)}$, the $n$-point rather than $m$-point version of it. Then
$r=(\bar q^\star/\bar q^{(1)})\cdot(\bar q^{(1)}/\bar q^{(2)})\cdot(\bar q^{(2)}/\bar q^{(3)})\cdot(\bar q^{(3)}/\bar q)$
telescopes, and the four factors are the four contributions below. The attribution depends on that ordering --- the effects are not independent, and a different insertion order splits the total differently --- so the factors are a decomposition of one measured quantity, not four separately meaningful errors.
...
\paragraph{Measured, in part.} Two of the four factors --- the prior floor and the snapshot quadrature --- can be estimated rather than bounded, because $\widehat\pi_n$ is a post-hoc pass: recomputing \eqref{eq:snapshot-denominator} at a much larger $m$ and at the observed prior share gives a reference against which those factors are evaluated at every particle. The remaining two, the truncation constant and the rejection/redraw conditioning, are \emph{not} captured by such a reference and are bounded rather than measured; the figures below are therefore a lower bound on the total mismatch. On the Cellular Potts production history ($33{,}748$ asynchronous evaluations across $48$ workers, two correlated dimensions, so that (c) is live), contribution (b) alone gives $\widehat\zeta=0.022$ with $r\in[0.92,1.80]$, and the reported posterior sits $1.1\%$ in total variation from the large-$m$ reference, with posterior means within $0.2\%$ and $1.1\%$ of a standard deviation and the effective sample size essentially unchanged ($0.213$ against $0.228$). On a one-dimensional Gaussian-mean history where the exact $n$-point mixture is computable, (a) and (b) together give $\widehat\zeta=0.042$ and a total-variation distance of $0.26\%$ from the exact-fidelity estimator, with the reported posterior $0.5\%$ narrower --- the tail-suppressing direction predicted by (a). Of contribution (d), the two events a stored history records --- a rejection fallback and an exhausted redraw --- did not occur once in $12{,}000$ draws; a redraw that succeeded on a later attempt leaves no trace, and is ruled out only by the argument that it needs a candidate to land where the mixture it came from has density below $10^{-12}$. The bound \eqref{eq:tilt-bound} is honest but loose against these direct measurements, by about a factor of two on the Cellular Potts run ($2.3\%$ against $1.1\%$) and by an order of magnitude on the one-dimensional history ($4.4\%$ against $0.26\%$), as sup-norm-free bounds valid for every $f$ generally are.
...
\end{equation}
Under sequential execution $\mathcal{F}_i=\sigma(\xi_1,\dots,\xi_i)$ serves, and \eqref{eq:app-conditional-law} holds by construction. Under \emph{asynchronous} execution the existence of such a filtration is a genuine assumption, not a bookkeeping detail, and we flag it as Assumption~\ref{ass:filtration}: a discrepancy $\rho_i$ may still be in flight when candidate $i+1$ is proposed, so the natural proposal-time $\sigma$-field does not contain it, and enlarging the filtration to include in-flight outcomes is legitimate only if those outcomes do not feed back into which rank proposes next --- which completion-time-driven scheduling does not guarantee. This is the same mechanism as the runtime-dependent censoring of \S\ref{sec:limitations}, item (iv), seen from the theory side. Concurrency also affects the reconstruction of the $q_{\tau_s}$ from a single rank's arrival-ordered log, which enters only through $\|\bar q_n-\bar q_\infty\|_\infty$ in Assumption~\ref{ass:stab}.

No structural form is imposed on $\tilde q_i$: it is whatever the archive rule, the capped rejection, the underflow redraws and the bootstrap phase jointly produce, and it enters the proofs only through $\bar q^\star_n=\tfrac1n\sum_{i\le n}\tilde q_i$ of \eqref{eq:draw-mixture}, for which the reported denominator $\bar q_n$ of \eqref{eq:snapshot-denominator} is an $m$-component stand-in. Let $\mathcal{Q}$ be the set of densities of the form \eqref{eq:snapshot-denominator} whose components satisfy Assumption~\ref{ass:class}. Elements of $\mathcal{Q}$ are parameterized by $p_q=(\text{means},\text{component weights},\text{covariances},\alpha)$ ranging over a compact set $P_q\subset\mathbb{R}^{D}$, $D=m\,(kd+k+d(d+1)/2)+m$.
...
\emph{Step 2 (drift).} Lemma~\ref{lem:condmean} with $r=1$, $\phi=\pi g$ applies to $V_i$ because $(\epsilon_{i-1},\bar q_{i-1})$ is $\mathcal{F}_{i-1}$-measurable and $g$ is a fixed function under the central-limit clause of Assumption~\ref{ass:stab}; with $\int\pi gL_{\epsilon_\infty}=0$,
\[
...
\ref{ass:filtration} adapted filtration & \textbf{assumed} & Automatic for a sequential run. Under asynchrony a discrepancy may be in flight when the next candidate is proposed, so the proposal-time $\sigma$-field does not contain it, and enlarging the filtration is legitimate only if completion times do not steer which rank proposes next --- which the scheduler does not guarantee. Same mechanism as Limitation~(iv). \\
...
\ref{ass:stab}(i) fidelity \eqref{eq:fidelity} & assumed; $r$ \emph{measured} & The one place the sampler enters. Its four contributions are decomposed in \S\ref{sec:theory-r} --- prior floor (closed form), snapshot quadrature ($\le V_n/m$), truncation constant, capped rejection and underflow redraws --- and $\zeta$ is estimated directly from stored histories rather than bounded: $\widehat\zeta=0.022$ on the Cellular Potts production run, where the reported posterior sits $1.1\%$ in total variation from a large-$m$ reference. \\
