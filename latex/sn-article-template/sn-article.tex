...
\end{equation}
$\bar q^\star_n$ being the mixture the pooled sample was actually drawn from. For the central limit theorem we require in addition that a (possibly random) limiting proposal $q_\infty$ exist, with $\bar q_\infty=\delta\pi+(1-\delta)q_\infty$, and that
\begin{equation}
\sqrt n\,\bigl(|\epsilon_n-\epsilon_\infty|+\|\bar q_n-\bar q_\infty\|_\infty\bigr)\to0,
\qquad
\tfrac{1}{\sqrt n}\textstyle\sum_{i\le n}\|q_i-q_\infty\|_\infty\to0
\qquad\text{a.s.}
\label{eq:rate}
\end{equation}
\end{assumption}
...
\paragraph{$\mathcal{A}_0$ is itself an idealization of the code.} Proposition~\ref{prop:tilt} isolates the defensive floor, and we are explicit that it does not capture the shipped sampler exactly; three further deviations remain unquantified. (a) The reject-resample fallback --- when a perturbation covariance wider than the prior box leaves the truncated mixture with no usable in-box mass, the candidate is drawn from the prior --- stamps the draw with the active bandwidth, so the retroactive pass classifies it as an archive-phase draw rather than a prior draw. (b) A candidate whose proposal-time denominator underflows is redrawn, which conditions the effective sampling law on the denominator exceeding a floor without a compensating weight correction. (c) Each truncated mixture component is normalized by the product of its per-axis in-box masses, exact for a diagonal perturbation covariance and for components far from every face of the box, but an approximation otherwise; the evaluated proposal density then differs from the density actually sampled by a bounded per-component factor. Each is rare or small in the regimes we run --- (a) and (b) fire only in degenerate proposal geometries, (c) only near a box face under a strongly correlated covariance --- but none is shown to vanish asymptotically, and a referee should read Proposition~\ref{prop:tilt} as covering the idealized $\mathcal{A}_0$, not the binary. Making (c) exact (a full-covariance box probability, or a diagonal perturbation covariance) and removing (a)--(b)'s conditioning are implementation changes, hence subject to the same re-run cost as the defensive draw itself.
...
\;\longrightarrow\;0
\]
almost surely, by the first half of \eqref{eq:rate}. This step, and only this step, is what the $\sqrt n$ rate on $(\epsilon_n,\bar q_n)$ buys.
...
\]
The second half of \eqref{eq:rate} is exactly $n^{-1/2}\sum_{i\le n}\|q_i-q_\infty\|_\infty\to0$, so $n^{-1/2}\sum_{i\le n}c_i\to0$. (Under the sufficient condition $\|q_i-q_\infty\|_\infty=O(i^{-1/2-a})$ this holds in each of the three cases $a<1/2$, $a=1/2$, $a>1/2$, where the partial sums are $O(n^{1/2-a})$, $O(\log n)$ and $O(1)$ respectively.)
...
\]
with an error at most a constant multiple of $\|q_i-q_\infty\|_\infty$; since \eqref{eq:rate} gives $n^{-1/2}\sum_{i\le n}\|q_i-q_\infty\|_\infty\to0$ and a fortiori $n^{-1}\sum_{i\le n}\|q_i-q_\infty\|_\infty\to0$, the Ces\`aro average $\tfrac1n\sum_{i\le n}\mathbb{E}_{i-1}U^2_i\to v^2$ a.s.; with $\tfrac1n\sum_ic^2_i\to0$ from Step 2,
...
\ref{ass:defensive} floor, sampler & \textbf{not satisfied} & The reported runs draw from the prior only in the bootstrap phase, so $\nu_n\downarrow0<\delta$; \eqref{eq:defensive-proposal} is the one-line change that removes this. Proposition~\ref{prop:tilt} isolates the consequence for the idealized $\mathcal{A}_0$; the code additionally (i) stamps a reject-resample prior fallback with the active bandwidth, so the retroactive pass counts it as an archive-phase draw, and (ii) redraws candidates whose proposal-time denominator underflows, conditioning the effective sampling law without a weight correction. Neither is shown to vanish asymptotically. \\
...
\ref{ass:stab} $\epsilon_\infty>0$ & assumed & Setting a strictly positive \texttt{min\_tol} forces $\epsilon_\infty\ge\texttt{min\_tol}>0$, since $\{\epsilon_n\}$ is non-increasing and clipped from below; it does not by itself show the floor is \emph{attained} in finite time, which is what would make the $\epsilon$ part of \eqref{eq:rate} hold trivially. The runs reported here leave \texttt{min\_tol} unset (zero is accepted), so the bandwidth their finite budget reaches is not an asymptotic $\epsilon_\infty$ at all. \\
