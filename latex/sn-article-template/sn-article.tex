...
\begin{assumption}[stabilization]\label{ass:stab} $\epsilon_n\downarrow\epsilon_\infty$ for a \emph{deterministic} $\epsilon_\infty>0$, and
\begin{equation}
...
\end{equation}
$\bar q^\star_n$ being the mixture the pooled sample was actually drawn from. For the central limit theorem we require in addition that a (possibly random) limiting proposal $q_\infty$ exist, with $\bar q_\infty=\delta\pi+(1-\delta)q_\infty$, and that, almost surely,
\begin{equation}
\begin{aligned}
...
\label{eq:rate}
\end{equation}
where $\epsilon_{i-1}$ is the bandwidth in force at the $i$-th proposal and $\bar q_{i-1}$ is \eqref{eq:snapshot-denominator} formed from the first $i-1$ draws --- both $\mathcal{F}_{i-1}$-measurable, with the convention $\bar q_0:=\pi$. The second condition is an \emph{aggregate} rate on these predictable proxies; it is what lets the central limit theorem accommodate a random $q_\infty$. The limiting \emph{proposal} may be random, but the limiting \emph{bandwidth} may not: $\pi_{\epsilon_\infty}$ must be a fixed distribution for ``$\widehat\pi_n(f)\to\pi_{\epsilon_\infty}(f)$'' to assert convergence to something, and the centred integrand $f-\pi_{\epsilon_\infty}(f)$ must be a fixed function for the martingale arrays below to be adapted. A configured bandwidth floor (\texttt{min\_tol}) supplies a deterministic $\epsilon_\infty$; a purely data-driven schedule does not.
\end{assumption}
...
\paragraph{Assumed versus true by construction.} Table~\ref{tab:assumption-status} in Appendix~\ref{app:theory} maps every assumption onto its status in the implementation. Assumptions~\ref{ass:kernel} and \ref{ass:defensive} hold by construction, the latter for the denominator but not (in the reported runs) for the sampler; Assumption~\ref{ass:class} holds up to the component normalizer. Assumption~\ref{ass:stab} is a genuine assumption, and the reported runs do not meet all of it: the top-$k$ archive rule does not obviously stabilize, since the archive keeps improving as better particles arrive, and we do not prove that it does; and its requirement of a deterministic limiting bandwidth asks for a configured \texttt{min\_tol} that those runs leave unset. Its consequences are what \S\ref{sec:results-validity}'s simulation-based calibration probes empirically. Two further approximations are absorbed into it rather than left dangling: under asynchronous execution the reporting rank's arrival-ordered log need not reproduce the proposal each candidate was actually drawn from, and the $m$-snapshot mixture is a coarse quadrature of the proposal path --- both enter only through $\|\bar q_n-\bar q_\infty\|_\infty$.
...
\emph{Step 2 (drift).} Lemma~\ref{lem:condmean} with $r=1$, $\phi=\pi g$ applies to $V_i$ because $(\epsilon_{i-1},\bar q_{i-1})$ is $\mathcal{F}_{i-1}$-measurable and $g$ is a fixed function ($\epsilon_\infty$ being deterministic, Assumption~\ref{ass:stab}); with $\int\pi gL_{\epsilon_\infty}=0$,
\[
...
\ref{ass:stab} $\epsilon_\infty>0$, deterministic & \textbf{not satisfied} & A strictly positive \texttt{min\_tol} forces $\epsilon_\infty\ge\texttt{min\_tol}>0$, since $\{\epsilon_n\}$ is non-increasing and clipped from below, and makes the limit a configured constant rather than a function of the run --- which the proof of Theorem~\ref{thm:clt} needs, because the centred integrand $f-\pi_{\epsilon_\infty}(f)$ must be a fixed function. Even then, attainment in finite time (which would make the $\epsilon$ part of \eqref{eq:rate} trivial) is not shown. The runs reported here leave \texttt{min\_tol} unset (zero is accepted), so their limiting bandwidth is neither configured nor deterministic, and the bandwidth a finite budget reaches is not an asymptotic $\epsilon_\infty$ at all. \\
