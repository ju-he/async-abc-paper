...
\item \textbf{Asymptotic guarantees, proved for a precisely specified algorithm.} We prove consistency and a central limit theorem for the defensive-mixture streaming estimator $\mathcal{A}_\delta$ (\S\ref{sec:theory}, proofs in Appendix~\ref{app:theory}): self-contained rather than a corollary of AMIS theory, with explicit rate conditions on the proposal sequence and bandwidth schedule, an explicit asymptotic variance that separates the simulator's own noise from the importance-sampling variance, and a consistent variance estimator giving per-run confidence intervals at no extra simulation cost. A persistent prior component of mass $\delta$, shared by sampler and denominator, bounds every importance weight by $1/\delta$ and replaces the two-sided density-ratio condition a local archive proposal cannot meet. We further show exactly what the bounded-memory estimator used in the experiments (a fixed $m$-snapshot denominator over a top-$k$ archive proposal) converges to, and bound its distance from the target; its calibration is in addition checked empirically by simulation-based calibration and matched-budget posterior error.
...
\begin{assumption}[stabilization]\label{ass:stab} $\epsilon_n\downarrow\epsilon_\infty>0$, and there is a (possibly random) density $q_\infty$ on $\Theta$ with $\|q_n-q_\infty\|_\infty\to0$ and $\|\bar q_n-\bar q_\infty\|_\infty\to0$ almost surely, where $\bar q_\infty=\delta\pi+(1-\delta)q_\infty$. For the central limit theorem we require in addition a \emph{rate}: for some $a>0$,
\begin{equation}
\|q_n-q_\infty\|_\infty+\|\bar q_n-\bar q_\infty\|_\infty+|\epsilon_n-\epsilon_\infty| \;=\; O\!\left(n^{-1/2-a}\right)\quad\text{a.s.}
\label{eq:rate}
\end{equation}
\end{assumption}

Assumption~\ref{ass:kernel} holds for the Gaussian kernel with $L_K=2/(e\,\epsilon_\infty)$ and for Epanechnikov with $L_K=2/\epsilon_\infty$; it excludes the hard indicator, which is not Lipschitz in $\epsilon$ --- one concrete reason the method uses smooth kernels. Assumption~\ref{ass:class} holds by construction in the implementation (Appendix~\ref{app:theory}, Table~\ref{tab:assumption-status}). Assumption~\ref{ass:stab} is the only genuinely restrictive one; it is where the adaptive nature of the scheme is quarantined, and \eqref{eq:rate} is the explicit rate the CLT needs --- a schedule as slow as $\epsilon_n-\epsilon_\infty\sim1/\log n$ violates it, and so does a proposal sequence that keeps drifting at scale $n^{-1/2}$.
...
Let Assumptions~\ref{ass:kernel}--\ref{ass:stab} hold for $\mathcal{A}_\delta$, including the rate \eqref{eq:rate}. Then for every bounded measurable $f$ with $\sigma_f^2>0$,
\begin{equation}
\sqrt{n}\bigl(\widehat\pi_n(f)-\pi_{\epsilon_\infty}(f)\bigr)\;\longrightarrow\;\mathcal{N}\!\left(0,\sigma_f^2\right)
\quad\text{stably},
\label{eq:clt-conv}
\end{equation}
where
...
Three readings of \eqref{eq:tilt-bound}. The tilt vanishes as $\delta\to0$, i.e.\ as the snapshot count $m$ grows. It is exactly zero whenever genuine prior draws already carry at least a $\delta$ share of the history, which is the regime of the short, expensive-simulator runs. And in the fast-simulator regime, where $\nu_\infty\approx0$ and $\delta\approx0.024$ at $m=20$, it is bounded by ${\approx}2.5\%$ in total variation over any region where the limiting proposal is at least as heavy as the prior, plus whatever ABC-posterior mass lies outside that region --- a small, one-sided, tail-suppressing distortion, in the same direction as the mild over-dispersion of the reported four-dimensional posterior (\S\ref{sec:results-validity}). Turning $\mathcal{A}_0$ into $\mathcal{A}_\delta$, and so removing the tilt, is a one-line change to the candidate draw \eqref{eq:defensive-proposal}; because it alters the sampler it changes the algorithm under test, so we state the gap here rather than re-running the campaign against it.

\paragraph{Assumed versus true by construction.} Table~\ref{tab:assumption-status} in Appendix~\ref{app:theory} maps every assumption onto its status in the implementation. Assumptions~\ref{ass:kernel}--\ref{ass:defensive} hold by construction, the last of them for the denominator but not (in the reported runs) for the sampler. Assumption~\ref{ass:stab} is a genuine assumption: the top-$k$ archive rule does not obviously stabilize, since the archive keeps improving as better particles arrive, and we do not prove that it does. Its consequences are what \S\ref{sec:results-validity}'s simulation-based calibration probes empirically. Two further approximations are absorbed into it rather than left dangling: under asynchronous execution the reporting rank's arrival-ordered log need not reproduce the proposal each candidate was actually drawn from, and the $m$-snapshot mixture is a coarse quadrature of the proposal path --- both enter only through $\|\bar q_n-\bar q_\infty\|_\infty$.

\paragraph{Fixed versus growing $m$.} The snapshot clause of Assumption~\ref{ass:stab} is where the estimator's cost meets its assumptions. With $m$ fixed, an $m$-point draw-proportional mixture tracks the empirical draw mixture $\tfrac1n\sum_{i\le n}q_i$ only if the proposal path settles: the error is at most $V_n/m$, where $V_n=\sum_{i<n}\|q_{i+1}-q_i\|_\infty$ is the path's total variation. Letting $m=m_n\to\infty$ with $V_n/m_n\to0$ discharges the clause without assuming stabilization, and the proofs go through unchanged for any $m_n=o(n)$ (Appendix~\ref{app:theory}, Remark~\ref{rem:growing-m}), at a retroactive pass of cost $\mathcal{O}(n\,m_n\,k)$ instead of $\mathcal{O}(nk)$. The literal cumulative mixture over every past proposal is the endpoint $m_n=n$, at $\mathcal{O}(n^2k)$. This is the precise form of the trade-off between the estimator's cost and the assumptions it needs; what we run is the fixed-$m$ end of it, and \S\ref{sec:implementation} reports the cost there.
...
\end{lemma}
\begin{proof}
Fix $p$. The variables $D^p_i:=\psi_p(\xi_i)-\mathbb{E}_{i-1}\psi_p(\xi_i)$ form a martingale difference sequence with $|D^p_i|\le2B$, so by the Azuma--Hoeffding inequality $\mathbb{P}(|\sum_{i\le n}D^p_i|\ge nt)\le2\exp(-nt^2/8B^2)$ for every $t>0$. Fix $\eta>0$ and let $P_\eta$ be a finite $\eta$-net of $P$, of cardinality at most $C_P\eta^{-D}$. A union bound gives $\mathbb{P}(\max_{p\in P_\eta}|\sum_{i\le n}D^p_i|\ge nt)\le2C_P\eta^{-D}\exp(-nt^2/8B^2)$, which is summable in $n$; by Borel--Cantelli, $\max_{p\in P_\eta}|\tfrac1n\sum_{i\le n}D^p_i|\to0$ a.s. For arbitrary $p\in P$ pick $p'\in P_\eta$ with $\|p-p'\|\le\eta$; then $|D^p_i-D^{p'}_i|\le2\Lambda\eta$ by the Lipschitz hypothesis (which also bounds the conditional expectations), so $\limsup_n\sup_{p\in P}|\tfrac1n\sum_{i\le n}D^p_i|\le2\Lambda\eta$ a.s. Intersecting the almost-sure events over $\eta=1/j$, $j\in\mathbb{N}$, gives the claim.
\end{proof}

\begin{lemma}[conditional-mean identity]\label{lem:condmean}
Let $h:\Theta\to\mathbb{R}$ be bounded measurable, $\epsilon\in[\epsilon_\infty,\epsilon_0]$, and $q$ a density with $q\ge\delta\pi$. Then under \eqref{eq:app-conditional-law},
\[
\mathbb{E}_{i-1}\!\left[\frac{\pi(\theta_i)h(\theta_i)K_\epsilon(\rho_i)}{q(\theta_i)}\right]
=\int_\Theta \pi\,h\,L_\epsilon\,\frac{q^\delta_i}{q}\,d\theta .
\]
\end{lemma}
\begin{proof}
Condition on $\mathcal{F}_{i-1}$ and integrate first over $\rho_i\sim p(\cdot\mid\theta_i)$, which gives the factor $L_\epsilon(\theta_i)=\mathbb{E}[K_\epsilon(\rho)\mid\theta_i]$, then over $\theta_i\sim q^\delta_i$. Fubini applies because the integrand is bounded by $\|h\|_\infty/\delta$ (Lemma~\ref{lem:bounded}).
\end{proof}
...
\label{eq:app-class}
\end{equation}
$P$ is compact and $\sup_P\|\psi\|_\infty\le\|h\|_\infty/\delta$ by the computation of Lemma~\ref{lem:bounded}. For the Lipschitz hypothesis: each truncated component density $\varphi(\theta;\mu_j,\Sigma)/Z(\mu_j,\Sigma)$ has parameter-derivatives bounded uniformly over $\theta\in\Theta$, because $\Theta$ is compact, $\sigma_-^2I\preceq\Sigma\preceq\sigma_+^2I$ bounds $\varphi$ and its derivatives, and the truncation mass is bounded below, $Z(\mu_j,\Sigma)\ge c_0>0$, uniformly for $\mu_j\in\Theta$ (a box contains at least a $2^{-d}$ fraction of any small ball centred at one of its points, and $\Sigma$ has bounded spectrum). Hence every $q\in\mathcal{Q}$ obeys $\delta\pi_-\le q\le q_+<\infty$ and $q\mapsto q(\theta)$ is Lipschitz in $p_q$ uniformly in $\theta$; so is $1/q$, since $q$ is bounded away from $0$. With $K_\epsilon$ Lipschitz in $\epsilon$ (Assumption~\ref{ass:kernel}) and $\pi h$ bounded, \eqref{eq:app-class} satisfies the hypotheses of Lemma~\ref{lem:uslln}. Note that $(\epsilon_n,\bar q_n)\in P$ for every $n$, and that $\bar q_\infty\ge\delta\pi$ by taking uniform limits.

\subsection{Proof of Theorem~\ref{thm:consistency}}\label{app:theory-consistency}
Fix a bounded measurable $h$ and let $\widehat p_n:=(\epsilon_n,\bar q_n)\in P$. On the almost-sure event of Lemma~\ref{lem:uslln} the bound holds simultaneously for all $p\in P$, so it may be evaluated at the random $\widehat p_n$:
\begin{equation}
\frac1n\sum_{i\le n}W_{i,n}h(\theta_i)
=\frac1n\sum_{i\le n}\mathbb{E}_{i-1}\psi_{\widehat p_n}(\xi_i)+o(1)
...
\end{equation}
using $\tfrac1n\sum_{i\le n}q^\delta_i=\bar q^\star_n$. Split the integral against $\int\pi hL_{\epsilon_\infty}$:
\[
...
\]
where $R:=(\delta\pi_++(1-\delta)q_+)/(\delta\pi_-)$ bounds $\bar q^\star_n/\bar q_n$, using $|L_{\epsilon_n}-L_{\epsilon_\infty}|\le L_K|\epsilon_n-\epsilon_\infty|$ (Assumption~\ref{ass:kernel} and Jensen) and $L_{\epsilon_\infty}\le1$. The first term vanishes because $\epsilon_n\to\epsilon_\infty$. For the second, $\|q_n-q_\infty\|_\infty\to0$ gives $\|\bar q^\star_n-\bar q_\infty\|_\infty\to0$ by Ces\`aro averaging, and $\|\bar q_n-\bar q_\infty\|_\infty\to0$ by Assumption~\ref{ass:stab}; since $\bar q_n\ge\delta\pi_-$,
$\|\bar q^\star_n/\bar q_n-1\|_\infty\le\|\bar q^\star_n-\bar q_n\|_\infty/(\delta\pi_-)\to0$.
Hence $\tfrac1n\sum_iW_{i,n}h(\theta_i)\to\int\pi hL_{\epsilon_\infty}$ a.s. Applying this with $h=f$ and with $h\equiv1$ and dividing --- legitimate since $Z_{\epsilon_\infty}=\int\pi L_{\epsilon_\infty}>0$ --- gives $\widehat\pi_n(f)\to\pi_{\epsilon_\infty}(f)$ a.s.\hfill$\square$
...
\emph{Step 1 (freezing the parameters).} Put $U_i:=\pi(\theta_i)g(\theta_i)K_{\epsilon_\infty}(\rho_i)/\bar q_\infty(\theta_i)$. Since $\bar q_n,\bar q_\infty\ge\delta\pi_-$ and $K\le1$,
\[
\Bigl|S_n-\tfrac{1}{\sqrt n}\sum_{i\le n}U_i\Bigr|
\le\sqrt n\,\|g\|_\infty\pi_+\left[\frac{L_K|\epsilon_n-\epsilon_\infty|}{\delta\pi_-}+\frac{\|\bar q_n-\bar q_\infty\|_\infty}{(\delta\pi_-)^2}\right]
=O\!\left(n^{-a}\right)\to0
\]
almost surely, by the rate \eqref{eq:rate}. This step, and only this step, is what the rate buys.

\emph{Step 2 (drift).} By Lemma~\ref{lem:condmean} with $q=\bar q_\infty$ and $\epsilon=\epsilon_\infty$, and $\int\pi gL_{\epsilon_\infty}=0$,
\[
c_i:=\mathbb{E}_{i-1}U_i=\int\pi gL_{\epsilon_\infty}\frac{q^\delta_i-\bar q_\infty}{\bar q_\infty}\,d\theta,
\qquad
|c_i|\le\frac{(1-\delta)\|g\|_\infty\pi_+|\Theta|}{\delta\pi_-}\,\|q_i-q_\infty\|_\infty .
\]
Under \eqref{eq:rate}, $\sum_{i\le n}|c_i|=O(\sum_{i\le n}i^{-1/2-a})=O(n^{1/2-a})$, so $n^{-1/2}\sum_{i\le n}c_i=O(n^{-a})\to0$.

\emph{Step 3 (martingale CLT).} Let $X_{n,i}:=n^{-1/2}(U_i-c_i)$, a martingale difference array with respect to the filtration $(\mathcal{F}_i)$, which does not depend on $n$ and is therefore nested. Two conditions:
\emph{(a) Conditional Lindeberg.} $|X_{n,i}|\le2\|g\|_\infty\pi_+/(\delta\pi_-\sqrt n)$ surely, so for every $\eta>0$ the truncated sum $\sum_i\mathbb{E}_{i-1}[X^2_{n,i}\mathbf{1}\{|X_{n,i}|>\eta\}]$ is exactly zero for all $n$ large. The defensive floor makes this step trivial where an unbounded-weight scheme would require a uniform-integrability argument.
\emph{(b) Conditional variance.} By Lemma~\ref{lem:condmean} applied to $\pi^2g^2K^2/\bar q^2_\infty$ (the same conditioning, integrating $K^2_{\epsilon_\infty}$ against $p(\cdot\mid\theta)$ to give $L^{(2)}_{\epsilon_\infty}$),
\[
\mathbb{E}_{i-1}U_i^2=\int\frac{\pi^2g^2L^{(2)}_{\epsilon_\infty}}{\bar q^2_\infty}\,q^\delta_i\,d\theta
\;\longrightarrow\;\int\frac{\pi^2g^2L^{(2)}_{\epsilon_\infty}}{\bar q_\infty}\,d\theta=v^2
\]
as $\|q_i-q_\infty\|_\infty\to0$, so the Ces\`aro average $\tfrac1n\sum_{i\le n}\mathbb{E}_{i-1}U^2_i\to v^2$ a.s.; with $\tfrac1n\sum_ic^2_i\to0$ from Step 2,
$\sum_i\mathbb{E}_{i-1}X^2_{n,i}=\tfrac1n\sum_i(\mathbb{E}_{i-1}U^2_i-c^2_i)\to v^2$ a.s.
The martingale central limit theorem for arrays with nested $\sigma$-fields \cite[Thm.~3.2 and Cor.~3.1]{hall1980martingale} then gives $\sum_iX_{n,i}\to\mathcal{N}(0,v^2)$ stably; the convergence is stable rather than merely distributional precisely because $v^2$ is $\mathcal{F}_\infty$-measurable and possibly random. Combining Steps 1--3 gives \eqref{eq:app-clt-target}.\hfill$\square$

\begin{proof}[Proof of Corollary~\ref{cor:ci}]
Write $\widehat\sigma^2_n=[\tfrac1n\sum_iW^2_{i,n}(f(\theta_i)-\widehat\pi_n(f))^2]\,/\,[\tfrac1n\sum_iW_{i,n}]^2$. In the numerator, $(f-\widehat\pi_n(f))^2=g^2+2g(\pi_{\epsilon_\infty}(f)-\widehat\pi_n(f))+(\pi_{\epsilon_\infty}(f)-\widehat\pi_n(f))^2$, and $W_{i,n}\le1/\delta$ with $\widehat\pi_n(f)\to\pi_{\epsilon_\infty}(f)$ a.s.\ (Theorem~\ref{thm:consistency}), so the cross terms vanish and it suffices to treat $\tfrac1n\sum_iW^2_{i,n}g(\theta_i)^2$. Apply Lemma~\ref{lem:uslln} to the class $\psi_{(\epsilon,p_q)}(\theta,\rho)=\pi(\theta)^2g(\theta)^2K_\epsilon(\rho)^2/q_{p_q}(\theta)^2$, which is bounded by $\|g\|^2_\infty/\delta^2$ and Lipschitz in $(\epsilon,p_q)$ by the same argument as \eqref{eq:app-class}; then Lemma~\ref{lem:condmean} and the argument of Step 3(b) give $\tfrac1n\sum_iW^2_{i,n}g^2(\theta_i)\to v^2$ a.s. The denominator tends to $Z^2_{\epsilon_\infty}$, so $\widehat\sigma^2_n\to v^2/Z^2_{\epsilon_\infty}=\sigma^2_f$ a.s. Because the convergence in Theorem~\ref{thm:clt} is stable and $\widehat\sigma_n\to\sigma_f$ with $\sigma_f$ $\mathcal{F}_\infty$-measurable, the ratio converges in distribution to a standard normal.
\end{proof}
...
\]
whose modulus is at most $2\|f\|_\infty\zeta/(1-\zeta)$ since $\pi_{\epsilon_\infty}(r_\infty)\ge1-\zeta$; taking the supremum over $\|f\|_\infty\le1$ and halving gives the total-variation bound.\hfill$\square$
...
\ref{ass:class} proposal class & by construction & The archive lies in the prior box, so component means do and $\Theta$ is compact; each component is truncated to the box with an analytic normalizing mass; the covariance carries an additive jitter $10^{-9}\mathrm{tr}(\widehat\Sigma)/d+10^{-12}\bar b^2$ ($\bar b^2$ the mean squared box width), whose second term is a run-independent floor, giving $\sigma_-^2>0$; $\sigma_+^2$ follows from the archive lying in a bounded box. \\
...
\ref{ass:defensive} floor, sampler & \textbf{not satisfied} & The reported runs draw from the prior only in the bootstrap phase, so $\nu_n\downarrow0<\delta$. Proposition~\ref{prop:tilt} states the consequence exactly; \eqref{eq:defensive-proposal} is the one-line change that removes it. \\
...
\ref{ass:stab} $\epsilon_\infty>0$ & configurable & Guaranteed when the bandwidth floor \texttt{min\_tol} is set, in which case the schedule reaches it in finite time and the $\epsilon$ part of \eqref{eq:rate} holds exactly. The runs reported here leave it unset and use the data-driven schedule, so $\epsilon_\infty$ is the bandwidth the finite budget reaches rather than a configured floor. \\
