...
\paragraph{Calibration under multimodality and in higher dimensions.} To probe exactly those settings, we run SBC (1000 trials, matched simulation budget) on two harder targets. The first is a \emph{multimodal} model $y_i\sim\mathcal N(\theta^2,\sigma^2)$: the sign ambiguity gives a symmetric bimodal posterior at $\pm\sqrt{\bar y}$, the case a top-$k$ archive is most at risk of collapsing onto a single mode. It does not collapse --- the archive retains \emph{both} modes in $85.7\%$ of trials and the mode containing the true value in $86.7\%$ (the remainder are largely near-$\theta{=}0$ trials where the two modes merge), the importance weights stay near-uniform (median effective-sample-size fraction $0.99$, median maximum normalised weight $0.011$), and coverage tracks the nominal level ($0.48/0.79/0.88/0.93$), calibrated within binomial error at the two lower levels with the same mild upper-level under-coverage seen in one dimension. The second is the \emph{four-dimensional} g-and-k model. Here both methods under-cover at the upper levels under the matched budget, and the asynchronous method more so (e.g.\ empirical coverage $0.85/0.89$ at the $0.90/0.95$ levels for the location parameter $A$, and lower for the shape parameters $B,k$); its rank distributions depart from uniform (chi-square $p<10^{-3}$) while the weights remain healthy (median effective-sample-size fraction $0.81$), so the miscalibration reflects an over-concentrated top-$k$ posterior in higher dimensions rather than weight degeneracy. Doubling the simulation budget (to $80{,}000$ per trial) leaves the coverage essentially unchanged --- $0.85/0.89$ at the $0.90/0.95$ levels for $A$, within Monte Carlo error for $B,g,k$ --- so the under-coverage is not budget-limited. It is a \emph{reporting-truncation} effect. Widening the reported support from the top-$k$ archive toward the whole evaluated history (the \emph{same} runs, resampling a larger set of the lowest-discrepancy records by the AMIS weight) raises coverage monotonically and continuously through nominal: for the location parameter $A$ it moves $0.85/0.89 \to 0.93/0.96 \to 0.97/0.99$ at the $0.90/0.95$ levels as the reported support grows from $200$ to $500$ to the full history, and the full history \emph{over}-covers at every level and parameter --- the finite-$\epsilon$ smooth-ABC posterior is over-dispersed at the loose bandwidth a four-dimensional budget reaches. The calibrated posterior is therefore \emph{bracketed} between the over-confident top-$k$ core and the over-dispersed full history, and an intermediate reported support recovers it ($M\approx400$--$800$; $B$ and $k$ are already near-nominal at $M{=}500$, e.g.\ $0.89/0.94$). So the four-dimensional miscalibration is a property of \emph{how much of the history is reported}, not of the AMIS weighting (whose importance weights stay well-conditioned) or the budget; the lever is the reported archive size, or the defensive-mixture floor (\S\ref{sec:limitations}). We accordingly report the streaming estimator as well calibrated in one dimension and robust to multimodality, and in four dimensions calibrated by the reported support --- over-confident at the top-$k$ archive reported by default, over-dispersed at the full history, near-nominal in between.
...
SBC (Gaussian, 1-D)    & 48 & 300  & 1000 & 100 & A,S \\
