...
# SITE_NASTJAPY_PATH and SITE_SCRATCH_ROOT based on $SYSTEMNAME (set by JSC on
...
#
# Honors SLURM_ACCOUNT_OVERRIDE / SLURM_PARTITION_OVERRIDE / NASTJAPY_PATH /
...
case "${SYSTEMNAME:-}" in
    jureca)
        ...
        SITE_NASTJAPY_PATH="/p/project1/eats-rna/$USER/nastjapy"
        SITE_SCRATCH_ROOT="/p/scratch/eats-rna/$USER/async-abc"
        ;;
    juwels)
        SITE_ACCOUNT="tissuetwin" ; SITE_PARTITION="batch"
        SITE_NASTJAPY_PATH="/p/project1/tissuetwin/herold2/nastjapy"
    ...
    *)
        ...
        echo "  Set SLURM_ACCOUNT_OVERRIDE / SLURM_PARTITION_OVERRIDE / NASTJAPY_PATH / SCRATCH_ROOT, or extend site_env.sh." >&2
...
SITE_NASTJAPY_PATH="${NASTJAPY_PATH:-$SITE_NASTJAPY_PATH}"
SITE_SCRATCH_ROOT="${SCRATCH_ROOT:-$SITE_SCRATCH_ROOT}"
export SITE_ACCOUNT SITE_PARTITION SITE_NASTJAPY_PATH SITE_SCRATCH_ROOT
