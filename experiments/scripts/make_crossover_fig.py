...
monotone in the simulator's cost and crosses one between about one and three
seconds per simulation. The per-simulation efficiency of the sampler (the
tolerance it reaches at a matched simulation count, second marker) does not
follow the cost: it is at or above the baseline's on every benchmark but the
one-dimensional analytic target.
"""
...
def draw(frames):
    ...
    fig, ax = plt.subplots(figsize=ps.fig_size(0.55, aspect=0.7))
    ...
    ax.plot(df["sim_cost_s"], df["throughput_ratio"], marker=ps.MARKERS["async"], color=ps.COLORS["async"],
            ls="-", label="throughput, async / sync (equal wall clock)")
    ax.plot(df["sim_cost_s"], df["per_simulation_ratio"], marker=ps.MARKERS["sync"], color=ps.COLORS["sync"],
            ls="--", label="per-simulation efficiency, $\\epsilon_{\\mathrm{sync}}/\\epsilon_{\\mathrm{async}}$ at matched $n$")
    for _, r in df.iterrows():
        ax.annotate(LABEL.get(r.benchmark, r.benchmark), (r.sim_cost_s, r.throughput_ratio),
                    textcoords="offset points", xytext=(4, -10), fontsize=6.5, color="0.25")
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel("cost of one simulation (s)")
    ax.set_ylabel("ratio, asynchronous over synchronous")
